...
%% Conjunto general de entrenamiento 
 nombreArchivoSetCompleto='aTSCompletoManchasSCF.csv';
 
 %Leer las 
 nombreArchivoTraining='aTrainingManchasS.csv';
 nombreArchivoTest='aTestManchasS.csv';
nombreArchivoScreen='screenManchasSCF.txt'; 
...
%
formatSpec='%s%f%f%f%f%f%f%f%f%f%f%s%s';
...
fprintf('PRUEBA # %i \n',prueba);
fprintf('-----------------------------------\n');
dividirConjuntosManchas( proporcionTraining, pathPrincipal, pathResultados, nombreArchivoSetCompleto, nombreArchivoTraining, nombreArchivoTest);
...
% 10 TOTAL DE PIXELES DE MANCHAS, 11 TOTAL DE MANCHAS
tablaDSTrainingCaracteristicas=tablaDSTraining(:,10:11);
...
% 10 TOTAL DE PIXELES DE MANCHAS, 11 TOTAL DE MANCHAS
tablaDSTestComparar=tablaDSTest(:,10:11);
...
for indiceTest=1:size(arrayTest)
    ...
    etiquetaComparar=char(table2array(tablaDSTest(indiceTest,13)));
